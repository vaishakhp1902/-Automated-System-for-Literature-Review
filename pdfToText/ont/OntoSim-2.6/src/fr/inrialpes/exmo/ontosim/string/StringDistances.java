/*
 * $Id: StringDistances.java 158 2017-04-18 16:21:10Z euzenat $
 *
 * Copyright (C) INRIA, 2003-2007, 2009, 2017
 ...
 * They return doubles
 *
 * Eventually similar results may be obtained by encapsulating other libraries:
 * https://github.com/tdebatty/java-string-similarity/releases
 * https://lucene.apache.org/core/4_4_0/suggest/org/apache/lucene/search/spell/StringDistance.html
 *
 * @author Jérôme Euzenat
 * @version $Id: StringDistances.java 158 2017-04-18 16:21:10Z euzenat $ 
 */
...
public class StringDistances {
     ...
     *
     * @param s1 and
     * @param s2 : two strings on which substring distance is computed
     * @return the substring distance
     */
     ...
     *
     * @param s and
     * @param t: two strings on which equality distance is computed
     * @return 0 if both string are equal, 1 otherwise
     */
     ...
     *
     * @param s and
     * @param t: two strings on which the Hamming distance is computed
     ...
     *
     * @param s and
     * @param t: two strings on which the Jaro measure is computed
     * @return the Jaro measure between both strings
     */
     ...
     *
     * @param s and
     * @param t: two strings on which the Jaro-Winkler measure is computed
     * @return the Jaro-Winkler measure between both strings
     */
     ...
     *
     * @param s and
     * @param t: two strings on which the n-gramm distance is computed
	...
	int nonred = 0; // number of non redundant ngrams in s1 and in s2
	int common = 0; // number of common ngrams in s1 and s2
	int l1 = s.length()-n+1;
	int l2 = t.length()-n+1;
	for( int j=0; j < l2; j++){
	    if ( !t.substring(j+1).contains( t.substring( j, j+n ) ) ) nonred++;
	};
	for( int i=0; i < l1 ; i++ ){
	    if ( !s.substring(i+1).contains( s.substring( i, i+n ) ) ) {
		nonred++;
		for( int j=0; j < l2; j++){
		    // If dupplicated, break
		    int k = 0;
	...
	return 1.0 - ((double)common/(double)(nonred-common));
    }
    */
	...
	for ( int j=0; j < l2; j++ ) {
	    ...
	    for ( ; w < l2; w++ ) {
		int k = 0;
		for( ; ( k < n ) && ( t.charAt(w+k) == t.charAt(j+k) ); k++);
		if ( k == n ) break; // redundant
	    }
	    if ( w == l2 ) nonred++; // non redundant
	};
	for( int i=0; i < l1 ; i++ ){
	    int w = i+1;
	    for ( ; w < l1; w++ ) {
		int k = 0;
		for( ; ( k < n ) && ( s.charAt(w+k) == s.charAt(i+k) ); k++);
		if ( k == n ) break; // redundant
	    }
	    if ( w == l1 ) { // non redundant
		nonred++;
		for( int j=0; j < l2; j++){
		    // If dupplicated, break
		    int k = 0;
     ...
     * @param s and
     * @param t: two strings on which the n-gramm distance is computed
     * @return: the ratio between the number of common n-grams occurences over the
     *         total number of n-gramm occurences in both strings. 
     */
    public static double ngramOccurDistance( String s, String t ) {
	int n = 3; // tri-grams for the moment
	if (s == null || t == null) {
	    //throw new IllegalArgumentException("Strings must not be null");
	    return 1.;
	...
	return 1.0 - ((double)found)/((double)(l1+l2));
    }
     ...
     *
     * @param s and
     * @param t: two strings on which the Needleman-Wunsch distance is computed
     * @param gap: see the definition
     * @return: the Needleman-Wunsch distance between both strings. 
    */
	...
	*/		
     ...
     *
     * @param s1 and
     * @param s2: two strings on which the Needleman-Wunsch distance is computed
     * @return: the SMOA distance between both strings. 
     */
